...
getGSEApLots <- function(GSEAfiles){
  ...
  ggsave(paste0(outfile,".",outdivice), plot = myplot, device = outdivice, scale = 1, dpi = "print", 
         width = 3700, height = length(unique(masterEAdf$pathway)) * 40 + 200, units = 'px')
  ...
  for (k in optNClust){
    ...
    ggsave(paste0(outfile,k,"Clusters.",outdivice), plot = myplot, device = outdivice, scale = 1, dpi = "print", 
           width = 3700, height = length(unique(masterEAdf2plot_cluster$pathway)) * 55, units = 'px')
    ...
    ggsave(paste0(outfile,k,"Clusters_EnrGenes.",outdivice), plot = myplot, device = outdivice, scale = 1, dpi = "print",
           width = 3700, height = length(unique(masterEAdf2plot_clusterGenes$pathway)) * 55, units = 'px')
  }
}  
