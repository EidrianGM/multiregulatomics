...
# safeQuant.R -i MQtoSQ_FAX.csv -l P445_AG_PolyA_40_20220617_gmin_FAX_p2_nonorm_Final --SP --SR --SM gMin --FN 2
...
paramsFile <- "/home/eidriangm/Desktop/toDo/surrey/multiregulatomics/data/old/P445_AG_PolyA_40_all_3_cond_20220611_Final/P445_AG_PolyA_40_20220617_gmin_UV_nonorm_p2_NOX_Final/myParams2.txt"
userOptionsDF <- read.delim(paramsFile,header = T)
userOptions <- as.list(userOptionsDF$value)
names(userOptions) <- userOptionsDF$param
userOptions <- type.convert(userOptions)
if (sum(is.na(c(userOptions$precursorMassFilter1,userOptions$precursorMassFilter2))) == 2){
  userOptions$precursorMassFilter <- NA
}else{
  userOptions$precursorMassFilter <- c(userOptions$precursorMassFilter1,userOptions$precursorMassFilter2)
}
...
# get file type
fileType <- .getFileType(userOptions$inputFile); cat(fileType)
...
if(fileType %in% c("ProgenesisProtein","ProgenesisFeature","ProgenesisPeptide")){
  ...
  if(!is.na(userOptions$expDesignTag)){
    expDesign0 <- expDesign
    colnames(expDesign0) <- c("class","isControl0")
    # user specified
    expDesign <- expDesignTagToExpDesign(userOptions$expDesignTag,expDesign0)
    expDesign <- cbind(expDesign,expDesign0)
    expDesign$isControl0 <- NULL
  }
...
any(grepl(userOptions$selectedProteinName,fData(eset)$proteinName,ignore.case=T))
...
if(exists("sqaPeptide")){
  ...
  write.table(out              , file=userOptions$peptideReportFilePath
